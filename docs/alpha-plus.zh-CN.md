...
- `local-development` 可显式配置 capability-rooted `workspace_list_directory`、
  `workspace_search_text`、`workspace_read_file` 与 `workspace_replace_text`：模型可列出根内 canonical relative directory
  ...
  重用或目标变化拒绝。所有结果仍按 exact tool completion 持久化后再进入下一模型步骤；没有
  shell、外部进程或隐式写权限。
...
- host 按项目既有统计口径通过 558 个 Rust 测试（connectors 14、deployment 8、knowledge 29、
  storage 248、runtime 48、API library 68、API main/config 6）与 28 个 Web Node 测试。
