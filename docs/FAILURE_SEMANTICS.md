...
- Run 成功只表示执行成功；需求回答的事实、遗漏和建议需人工接受。工具审批不代表接受回答。
- 组织模型配置编辑和供应商密钥轮换要求 Organization 管理权限，Workspace 管理权限不能代替。两者使用现有 revision 前置条件，冲突不自动重试覆盖。页面不返回输入密钥，更新模型超时时保留其他参数。
- Runtime 在执行/恢复时加载当前模型配置及密钥；不可变 Agent/Workflow 版本不等于模型连接快照。变更需隔离时创建新模型配置。
- Run 失败和取消页面展示恢复指引。预算调整需发布新 Workflow 并从工作项启动；普通重试创建新的 Run，保留原执行记录。
...
- Setup Token 错误返回 `401`。已有平台 Owner 时返回 `409`。并发 Setup 只有取得 advisory lock 且先提交的事务成功。
...
验收写入和工作项 revision 递增原子提交。过期或重复提交的 revision 返回 412，客户端刷新并重新核对后决定是否再提交。非成功、无输出或不关联当前工作项的 Run 返回 409；无用户或 Workspace 操作权限拒绝。验收记录只追加，后续意见作为新记录保留；验收不会改变运行结果或自动完成工作项。读失败时页面提示不可用，不将无法读取解释为已接受。
