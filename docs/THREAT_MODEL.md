...
| ZI-09 | High | 平台 Owner 借平台角色直接读取租户业务数据。 | `platform_owner` 独立存储；业务访问仍需绑定 Session 的限时 Grant、Organization/Workspace Context 和 RLS。`db/migrations/0030_platform_owner_role.sql`、`apps/zeus-api/src/control_plane/platform_tenants.rs` | 生产数据库执行平台 Owner 跨租户负面测试；平台运维 SQL 需审计。 |
...
| ZI-23 | Critical | 平台角色伪造租户 Context，绕过 Membership 或长期保留对租户数据的访问。 | 只允许绑定 Web Session 的 60 分钟 Grant；每个请求校验 PostgreSQL 状态；Actor、Grant ID 和原因进入 Audit 与 Security Event；租户 SQL 继续使用 `zeus_http` 与 RLS。真实数据库测试覆盖错误 Session、跨 Organization Workspace、撤销、无 Membership 访问和 RLS。 | 在托管 PostgreSQL 的实际角色链上重复负面测试，并验证多副本 Grant 到期行为。 |
| ZI-24 | High | JIT 角色迁移把普通联合用户错误提升为 Organization/Workspace Owner。 | JIT 默认 `member`，只有邀请或 Group Mapping 可以指定角色；`admin` 数据迁移不改变无映射默认值。K1/K2 测试覆盖角色迁移和跨 Organization Binding。 | 用真实企业 IdP 复测恶意 Group Claim、邀请和 Domain JIT 组合。 |
| ZI-25 | High | Binding 的 Organization 与 Provider 不一致，攻击者借另一个租户的 Provider 建立信任。 | `(organization_id, provider_id)` 复合外键拒绝交叉引用；Provider Token 完整校验后才解析全局身份。真实数据库测试覆盖同一 Google 风格身份的两个独立 Organization Binding、跨 Organization Context 拒绝和单 Binding 撤销。 | 用真实双 Provider 环境复测 callback mix-up 和显式绑定。 |
| ZI-26 | High | 外部链接触发 GET Workspace 切换，轮换用户 Session 并让旧标签页向错误租户提交数据。 | 只允许带 Origin/CSRF 的 Context POST；URL 不一致进入选择页；写操作返回 `workspace_context_changed`；BroadcastChannel 只做 UX 通知。Web 测试覆盖 Context 变化、跨 Workspace return path 和零/一/多个 Workspace 入口。 | 生产 HTTPS 下复测双标签页、Cookie flags 和代理 Header。 |
| ZI-27 | High | `platform_managed` 只隐藏导航，Organization Owner 仍能直接调用身份设置 API。 | 服务端权限与 RLS 拒绝 Owner 的受管身份设置访问；Organization 身份设置 layout 在加载子资源前拒绝访问；平台支持路径要求有效 Grant，并写 Audit 与 Security Event。真实数据库测试覆盖受管 Provider 的 Grant/RLS 访问和错误 Session 拒绝。 | 在生产角色和网关配置下，对 Provider、Domain、Identity Policy 和 OIDC Client 重跑直接 API 负面矩阵。 |
| ZI-28 | High | Suspend 只改变 Organization 状态，Runtime 仍领取 queued Run，或下游把旧 Access Token 当成长期有效。 | `0029_tenant_state_execution_guard.sql` 让 claim 只选择 active Organization；Suspend 请求取消未完成 Run，并阻断新写入、Schedule/Webhook、联合登录、Authorization 和 Refresh。OIDC 测试确认 Access Token 的 `exp - iat` 固定为 300 秒。 | 需要即时撤销时增加下游撤销查询或网关策略；生产多副本执行 claim、恢复和 Token 过期故障注入。 |
