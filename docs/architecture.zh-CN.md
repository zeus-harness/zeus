...
- `subagents`：`spawn_agent` / `list_agents` / `get_agent_result` 工具契约、严格参数边界、
  确定性 child identity 与有界结果；durable scope、原子 admission、目录分页和终态结果读取由
  runtime/storage 掌握，不由通用 executor 接受调用方自报身份。
...
`needs_attention` 不返回 partial output。当前阶段不提供 send、interrupt 或递归全图加载。
...
- 当前自动化按项目既有统计口径是 696 个 Rust 测试（其中 connectors 22、deployment 8、knowledge 29、LLM unit 30、
  provider contract 18、skills 5、subagents 8、storage 286、workflows 21、runtime 52、API library 95、API main/config 18）和 28 个 Web Node 测试全部通过；Rust fmt/clippy、Svelte
