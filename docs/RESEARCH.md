...
| 源码位置 | 看到的约束 | Zeus 的处理 |
| --- | --- | --- |
| `src/api_types/src/sessions.rs`、`src/middlewares/src/principal.rs`、`csrf_protection.rs` | Session 有显式状态、过期时间和最近活动时间。Cookie 请求经过 CSRF 与浏览器安全头检查。 | `web_sessions` 保存认证方法、空闲/绝对过期和 CSRF 摘要。Cookie 写请求同时校验 Origin、双提交 CSRF 和数据库摘要。 |
