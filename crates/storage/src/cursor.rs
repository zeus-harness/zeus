...
const AGENT_SUBAGENTS_KIND: &str = "agent_subagents";
...
pub(crate) fn encode_agent_subagents(
    account_id: &str,
    actor_user_id: &str,
    parent_session_id: &str,
    created_at: &str,
...
pub(crate) fn decode_agent_subagents(
    value: &str,
    account_id: &str,
    actor_user_id: &str,
    parent_session_id: &str,
...
mod tests {
    ...
    fn cursor_kind_account_actor_and_parent_are_not_interchangeable() {
        ...
        let subagents = encode_agent_subagents(
            "acc-a",
            "actor-a",
            "session-a",
            "2026-01-01T00:00:00Z",
        ...
        .unwrap();
        assert_eq!(
            decode_agent_subagents(&subagents, "acc-a", "actor-a", "session-a").unwrap(),
            TextKeyCursor {
                first: "2026-01-01T00:00:00Z".into(),
                second: "session-child".into(),
            }
        );
        assert!(decode_session_forks(&subagents, "acc-a", "actor-a", "session-a").is_err());
    }
