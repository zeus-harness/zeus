    ...
    <Card.Root class="mt-8">
      <Card.Header class="flex-row items-start justify-between gap-4">
        ...
        <form method="GET" class="flex items-center gap-2">
          <label class="sr-only" for="approval-status">审批状态</label>
          <select id="approval-status" name="status" value={data.filterStatus} class="h-9 rounded-md border border-input bg-background px-2 text-sm">
            ...
            <Table.Body>
              {#each approvals as approval (approval.id)}
                <Table.Row>
                  ...
                  <Table.Cell>
                    <a class="font-mono text-xs hover:underline" href={`/runs/${approval.run_id}`}>{approval.run_id}</a>
                    ...
                    {#if approval.status === 'pending'}
                      <div class="flex min-w-56 flex-col items-end gap-2">
                        <form method="POST" action="?/decide" class="flex w-full gap-2">
                          ...
                          <input name="reason" aria-label="批准理由" placeholder="可选理由" class="h-8 min-w-0 flex-1 rounded-md border border-input bg-background px-2 text-xs" />
                          <Button type="submit" size="sm">批准</Button>
                        </form>
                        <form method="POST" action="?/decide" class="flex w-full gap-2">
                          <input type="hidden" name="approval_id" value={approval.id} />
                          <input type="hidden" name="decision" value="reject" />
                          <input name="reason" aria-label="拒绝理由" placeholder="拒绝理由（可选）" class="h-8 min-w-0 flex-1 rounded-md border border-input bg-background px-2 text-xs" />
