...
import { decideApproval, listApprovals, loadWorkspaceData } from '$lib/api/workspace';
...
export const actions: Actions = {
  decide: async (event) => {
    ...
    redirect(303, `/approvals?status=${encodeURIComponent(event.url.searchParams.get('status') || 'pending')}`);
  }
};
