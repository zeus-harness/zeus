...
<main class="mx-auto max-w-[1480px] px-5 py-8 lg:px-8 lg:py-10">
  {#if data.trace.status !== 'ready' || !trace}
    ...
    />
  {:else}
    <a class="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground" href="/runs"><ArrowLeft class="size-4" />返回运行</a>
    <div class="mt-5 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
      <div class="min-w-0">
        ...
        {#if trace.run.work_item_id}
          <a class="mt-2 inline-flex text-sm text-primary hover:underline" href={`/work-items/${trace.run.work_item_id}`}>返回关联 WorkItem</a>
        {:else}
