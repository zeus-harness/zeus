...
export const load: PageServerLoad = async ({ fetch, parent, request, url }) => {
  ...
  const workspaceContext = { authStatus: auth.status, workspaceId: auth.principal?.workspace_id };
  const requestOptions = {
    apiBaseUrl: env.ZEUS_API_URL,
    workspaceId: auth.principal?.workspace_id ?? ''
  };
  ...
  return { myWorkItems, blockedWorkItems, approvals, recentRuns };
};
