...
export const load: PageServerLoad = async ({ fetch, parent, request, params, url }) => {
  ...
  return {
    ...
    runId: params.run_id,
    workspaceId: params.workspaceId,
    streamUrl: runEventStreamUrl({ workspaceId: params.workspaceId }, params.run_id)
  };
};
...
export const actions: Actions = {
  cancel: async (event) => {
    ...
    redirect(303, `/${event.params.workspaceId}/runs/${event.params.run_id}`);
  },
  retry: async (event) => {
    const context = await actionWorkspace(event);
    if (context.error) return context.error;
    ...
    redirect(303, `/${event.params.workspaceId}/runs/${retryRunId}`);
  },
  decide: async (event) => {
    const context = await actionWorkspace(event);
    if (context.error) return context.error;
