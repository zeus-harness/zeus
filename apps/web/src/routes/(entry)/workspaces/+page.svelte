<script lang="ts">
  ...
  const roleNames: Record<string, string> = { owner: '所有者', builder: '构建者', member: '成员', viewer: '只读成员', support: '平台支持' };
  const stateNames: Record<string, string> = { active: '可用', suspended: '已暂停', archived: '已归档' };
...
<svelte:head><title>Zeus · 选择工作空间</title></svelte:head>
  ...
  <div class="mx-auto max-w-5xl">
    <div class="flex flex-col gap-4 border-b border-border pb-7 sm:flex-row sm:items-end sm:justify-between">
      <div>
        ...
        <h1 class="mt-2 text-3xl font-semibold tracking-tight">选择工作空间</h1>
        <p class="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
          选择要进入的工作空间，继续处理其中的任务。
        </p>
      </div>
    ...
    {#if data.workspaces.length === 0}
      <Card.Root class="mt-8">
        <Card.Header>
          <Card.Title>还没有可用工作空间</Card.Title>
          <Card.Description>接受组织邀请，或联系组织所有者为你分配工作空间。</Card.Description>
        </Card.Header>
      </Card.Root>
    {:else}
          ...
          <Card.Root class="transition-colors hover:border-foreground/30">
            <Card.Header>
              <div class="flex items-start justify-between gap-4">
                ...
                <Badge variant={workspace.status === 'active' ? 'secondary' : 'outline'}>{stateNames[workspace.status] ?? workspace.status}</Badge>
              </div>
            </Card.Header>
            <Card.Content class="flex items-center justify-between gap-3 text-sm text-muted-foreground">
              <span>工作空间{roleNames[workspace.role] ?? workspace.role}</span>
              <span>组织{roleNames[workspace.organization.organization_role ?? 'support'] ?? workspace.organization.organization_role}</span>
            </Card.Content>
            <Card.Footer>
              <form
              ...
              >
                ...
                <Button type="submit" class="w-full" disabled={workspace.status !== 'active'}>
                  {workspace.id === data.autoSelect ? '正在进入…' : '进入工作空间'}
