      ...
      <Card.Content>
        <form method="POST" action="?/create" class="space-y-4">
          <div>
            ...
            <input id="source_run_id" name="source_run_id" required class="mt-2 h-10 w-full rounded-md border border-input bg-background px-3 font-mono text-xs" />
          </div>
          <div>
            <label class="text-sm font-medium" for="proposed_scope">发布范围</label>
            <select id="proposed_scope" name="proposed_scope" value="workspace" class="mt-2 h-10 w-full rounded-md border border-input bg-background px-3 text-sm">
          ...
          <div>
            ...
            <textarea id="content" name="content" required rows="5" class="mt-2 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"></textarea>
          </div>
          <div>
            <label class="text-sm font-medium" for="tags">Tags</label>
            <input id="tags" name="tags" placeholder="逗号分隔，可选" class="mt-2 h-10 w-full rounded-md border border-input bg-background px-3 text-sm" />
          </div>
          <div>
            <label class="text-sm font-medium" for="evidence">Evidence JSON</label>
            <textarea id="evidence" name="evidence" required rows="5" class="mt-2 w-full rounded-md border border-input bg-background px-3 py-2 font-mono text-xs" placeholder="JSON 数组：每项包含 event_kind 和 event_id">[]</textarea>
              ...
              {#each candidates as candidate (candidate.id)}
                <article class="rounded-xl border border-border p-4">
                  ...
                  <div class="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                    <a class="font-mono hover:underline" href={`/runs/${candidate.source_run_id}`}>Source Run {candidate.source_run_id}</a>
                  ...
                  {#if candidate.status === 'pending'}
                    <form method="POST" action="?/review" class="mt-4 flex flex-col gap-2 sm:flex-row">
                      ...
                      <input name="reason" placeholder="审阅理由（可选）" class="h-9 min-w-0 flex-1 rounded-md border border-input bg-background px-3 text-xs" />
        ...
        <Card.Content>
          <form method="GET" class="mb-6 grid gap-3 rounded-lg border border-border p-4 md:grid-cols-[minmax(0,1fr)_10rem_auto] md:items-end">
            <div>
              ...
              <input id="q" name="q" value={data.query} placeholder="搜索标题、内容或 tags" class="mt-2 h-10 w-full rounded-md border border-input bg-background px-3 text-sm" />
            </div>
            <div>
              <label class="text-sm font-medium" for="scope">Scope</label>
              <select id="scope" name="scope" value={data.scope} class="mt-2 h-10 w-full rounded-md border border-input bg-background px-3 text-sm">
            ...
            <div class="space-y-3">
              {#each entries as entry (entry.id)}
                <article class="rounded-xl border border-border p-4">
                  ...
                  <div class="mt-3 flex flex-wrap gap-3 text-xs text-muted-foreground"><span>published {dateLabel(entry.published_at)}</span><span>source <a class="font-mono hover:underline" href={`/runs/${entry.source_run_id}`}>{entry.source_run_id}</a></span></div>
                  ...
                  {:else}
                    <form method="POST" action="?/withdraw" class="mt-4 flex flex-col gap-2 sm:flex-row">
                      ...
                      <input name="reason" required placeholder="撤回理由" class="h-9 min-w-0 flex-1 rounded-md border border-input bg-background px-3 text-xs" />
