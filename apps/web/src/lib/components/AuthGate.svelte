<script lang="ts">
	...
	import type { BootstrapRequest, LoginRequest } from '$lib/types';
	...
	let { configured, onBootstrap, onLogin }: Props = $props();
...
<main class="bg-zeus-bg text-zeus-text px-5 py-10 grid min-h-dvh place-items-center">
	<section class="w-full max-w-[380px]" aria-labelledby="auth-title">
		...
		<h1 id="auth-title" class="font-semibold text-2xl tracking-[-0.025em]">
			{configured ? 'Sign in' : 'Set up your owner account'}
		</h1>
		...
		<form class="mt-7 gap-4 grid" onsubmit={submit}>
			...
			<Button class="mt-1 h-10 rounded-xl" type="submit" disabled={submitting}>
				{submitting ? 'Please wait…' : configured ? 'Sign in' : 'Create owner account'}
			</Button>
		</form>
	</section>
</main>
