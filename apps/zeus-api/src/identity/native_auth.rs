...
pub async fn native_login(
    ...
    let Ok(email) = normalize_email(&request.email) else {
        ...
        state.platform.metrics.record_identity_password_failure();
    ...
    .bind(ttl_seconds(state.identity.session_idle_ttl))
    .bind(ttl_seconds(state.identity.session_absolute_ttl))
    .fetch_one(&state.platform.database)
    .await?;
...
pub async fn verify_mfa(
    ...
    let method = match verify_second_factor(&state, user_id, &request.code).await {
        ...
        Err(ApiError::Unauthorized) => {
            ...
            state.platform.metrics.record_identity_mfa_failure();
            return Err(ApiError::Unauthorized);
        }
...
pub async fn list_web_sessions(
    ...
    let mut transaction = begin_user(&state.platform.database, user_id).await?;
...
pub(crate) async fn queue_email_verification(
    ...
    let mut connection = state.platform.database.acquire().await?;
...
fn throttle_key(state: &AppState, domain: &str, value: &str) -> Vec<u8> {
    privacy_digest(&state.identity.identity_hash_key, domain, value)
}
