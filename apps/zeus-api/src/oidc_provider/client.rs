...
fn validate_redirect_uris(
    ...
    for value in values {
        ...
        let local_deployment = state.public_url.scheme() == "http"
            && state.public_url.host_str().is_some_and(|host| {
                host.eq_ignore_ascii_case("localhost")
                    || host
                        .parse::<std::net::IpAddr>()
                        .is_ok_and(|ip| ip.is_loopback())
...
mod tests {
    ...
    use crate::{AppState, crypto::LocalEnvelopeCipher, supervisor::SupervisorMetrics};
