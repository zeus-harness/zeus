...
Schema v31 adds a durable FIFO inbox for ordinary user follow-ups. A follow-up
can be accepted while the Session is `ready`, `running`, or
`needs_attention` without changing the Session event sequence. The worker
consumes only the oldest queued item for a `ready` Session and atomically turns
it into the normal user turn, Agent, first model job, and `user_message` event.
Queued input survives restart, shares the existing actor/account/global reply
...
Schema v37 adds immutable `session_followup_sources` provenance for Agent-to-Agent
messages. It distinguishes parent-to-child `send_message` from child-to-parent
`report`, binds each source to the exact started tool call and source
Session/Agent, and keeps that evidence after FIFO claim and restart. Manifest
...
All require the exact persisted
...
`report@1-durable-parent-followup` is the reverse channel. A child supplies
only bounded report content; storage derives its unique direct parent from the
immutable spawn lineage and revalidates the exact started child tool scope.
The deterministic acknowledgement is a durable parent FIFO message, not a
read receipt. It wakes the parent for its next normal turn, survives restart,
routes a nested child's report exactly one edge upward, and does not end the
reporting child turn.
...
- Schema v4 adds durable Sessions and their ledger. Schema v5 adds local users,
  ...
  Schema v31 adds `session_followups` and actor-scoped
  `session_followup_receipts`. Enqueue is a sequence-CAS, idempotent operation
  but does not append a Session event; FIFO claim later rechecks the exact
  membership revision and binds the row to its real Session turn and Agent in
  one transaction. Startup, successful turn completion, explicit resume, and
...
- `POST /api/v1/sessions/{session_id}/turns` accepts
  ...
  SSE.
- `POST /api/v1/sessions/{session_id}/followups` accepts the same
